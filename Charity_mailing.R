...
pairs.panels(charity[2:21]) # check for correlated variables

# View distribution of variables
hist(hinc) # uniform across 7 categories, except spike for 4
hist(wrat) # highly negative skew toward 8, 9 levels
hist(avhv) # slight positive skew, log() normalizes well
hist(incm) # slight positive skew, log() normalizes okay
hist(inca) # slight positive skew, log() normalizes well
hist(plow) # strong positive skew, standard transformations don't work
hist(npro) # slight positive skew, sqrt() normalizes okay
hist(tgif) # strong positive skew, log() or log10() normalizes okay
hist(lgif) # strong positive skew, log() normalizes okay
hist(rgif) # strong positive skew, log() normalizes okay
hist(tdon) # strange skew w/plateu from 13-23, log() normalizes okay
hist(tlag) # strong positive skew, not sure how to best transform, maybe log
hist(agif) # strong positive skew, log() normalizes okay

table(reg1) # 1 indicates individual belongs to a region
table(reg2) # most populated
table(reg3) # least populated
table(reg4)
table(home) # 1 for homeowners, 6:1 ratio
...
# Box-Cox
start = round(1/6,5);
lambda = boxcoxfit(Y, add.to.data = start )$lambda 
...
model.dt <- rpart(donr ~., data=data.train.std.c, method="class")
par(mfrow=c(1,1))
